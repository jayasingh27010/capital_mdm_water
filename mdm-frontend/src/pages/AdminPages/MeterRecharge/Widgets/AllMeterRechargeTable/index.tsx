...
import { viewAllManualRechargeTable } from "src/api/Pages/meterRecharge";
...
const tableId = "manualRechargeTable";

export const manualRechargeTable: React.FC = () => {
  const { data, isLoading } = useApi(viewAllManualRechargeTable);
  ...
  const apiReference = (filters: any): Promise<any> => {
    return new Promise((resolve) => {
      viewAllManualRechargeTable(filters).then(({ data }: any) => {
        resolve({
