...
import { viewAllCreditNoteTable } from "src/api/Pages/meterRecharge";
...
const tableId = "creditNoteTable";

export const creditNoteTable: React.FC = () => {
  const { data, isLoading } = useApi(viewAllCreditNoteTable);
  ...
  const apiReference = (filters: any): Promise<any> => {
    return new Promise((resolve) => {
      viewAllCreditNoteTable(filters).then(({ data }: any) => {
        resolve({
