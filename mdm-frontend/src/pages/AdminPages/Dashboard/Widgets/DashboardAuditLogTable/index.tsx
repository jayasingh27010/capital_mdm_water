...
import { viewDashboardAuditLogTable } from "src/api/Pages/Dashboard"
...
const tableId = "dashboardAuditLogTable"


const DashboardAuditLogTable: React.FC = () => {
    const { data, isLoading } = useApi(viewDashboardAuditLogTable)
    const [defaultFilters, setDefaultFilters] = useState<any>({})
    const { dispatch } = useContext<any>(AppContext)

    const apiReference = (filters: any): Promise<any> => {
        return new Promise((resolve) => {
            viewDashboardAuditLogTable(filters)
            .then(({ data }: any) => {
                resolve({
...
export default DashboardAuditLogTable